...
public class BirdQuizGUI extends JFrame implements ActionListener {
    ...
    private JPanel optionsPanel;      // keep a handle so we can hide/show cleanly
    private boolean iconsReady = false; // marks if button icons are preloaded
    private static final String SUBMIT_SFX = "submit_button_pressed.wav";
    ...
    public BirdQuizGUI(String tableName, String serialPortName, String firstName, String email) {
        ...
        try {
        loadAndResizeImages();   // this only loads /buttons/* icons in your code
        iconsReady = true;
        } catch (IOException ex) {
        ex.printStackTrace(); // still safe to continue; you'll just see text-only radios
        }
...
private void setupOptionsPanel(GridBagConstraints gbc) {
    ...
    optionsPanel.setVisible(false); // hide until first question is bound

    if (options == null || options.length != 3) options = new JRadioButton[3];
    if (group == null) group = new ButtonGroup();

    for (int i = 0; i < options.length; i++) {
        if (options[i] == null) options[i] = new JRadioButton("");

        makeFlat(options[i]);
        options[i].setFont(RADIO_BUTTON_FONT);
        options[i].putClientProperty("index", i);
        options[i].addActionListener(this);
        options[i].setIconTextGap(12);

        // If your button images are already loaded, apply them now to avoid flash
        if (i < resizedImages.length && resizedImages[i] != null) {
            options[i].setIcon(resizedImages[i]);
        }
        if (i < selectedResizedImages.length && selectedResizedImages[i] != null) {
            options[i].setSelectedIcon(selectedResizedImages[i]);
            options[i].setRolloverIcon(selectedResizedImages[i]); // nice hover effect
        }
    ...
    /** Submit button row. */
// Submit button row (plays resources/sounds/submit.wav on press)
private void setupSubmitPanel(GridBagConstraints gbcParent) {
    if (submitPanel == null) {
        ...
        submitPanel.setBorder(new javax.swing.border.EmptyBorder(0, 0, 8, 0)); // lift a bit
    } else {
        submitPanel.removeAll();
    }

    if (submitButton == null) submitButton = new JButton("Submit");
    submitButton.setPreferredSize(new Dimension(200, 56));
    ...
    // central wiring (adds sound + submit)
    ensureSubmitIsWired();
    ...
    // Apply the colored button images to the 3 option radio buttons.
    private void applyOptionIcons() {
        if (options == null)
            return;
        for (int i = 0; i < options.length; i++) {
            ...
            // normal + selected icons (null-safe)
            if (i < resizedImages.length && resizedImages[i] != null) {
  ...
  private void setupScoreLabel(GridBagConstraints gbc) {
    ...
    gbc.gridwidth = 3;
    gbc.weightx = 1.0;
    gbc.weighty = 0.0;                   // don't let this row expand
    gbc.anchor = GridBagConstraints.CENTER;
    gbc.fill   = GridBagConstraints.NONE;
    gbc.insets = new Insets(4, 0, 0, 0); // small space above the score
    ...
    private JPanel buildHeaderBar() {
        ...
        // Right: countdown ring ONLY (the ring already draws mm:ss inside)
   ...
   private void initializeDatabaseConnection() {
    try {
        ...
        // Safe UI update: show current "To Beat" after we have a connection
        SwingUtilities.invokeLater(this::refreshToBeatLabel);

    } catch (ClassNotFoundException | SQLException e) {
        e.printStackTrace();
        connection = null;

        // Let the user know and keep UI benign
        ...
        SwingUtilities.invokeLater(() -> {
            if (toBeatLabel != null) toBeatLabel.setText("To Beat: —");
        });
    }
}
    ...
    private boolean initializeQuestionsAndImages() {
        try {
            ...
            loadAndResizeImages();
            // After loading button/submit images, apply them to existing UI:
            SwingUtilities.invokeLater(() -> {
                applyOptionIcons();
                applySubmitIcon(); // keep your white submit glyph applied too
                revalidate();
                repaint();
            });
   ...
   // =============================================================================
    // Section 7.3 quiz scores to beat 
    // =============================================================================
// --- High-score model (you already have this; OK to keep yours) ---


// Save one attempt to quiz_attempts (schema: quiz_name, score, total_questions, duration_seconds, finished_at)
private void saveAttemptToAttemptsTable(int durationSeconds) {
    if (connection == null) return;
    final String sql =
        "INSERT INTO quiz_attempts (quiz_name, score, total_questions, duration_seconds) " +
        "VALUES (?, ?, ?, ?)";
    try (PreparedStatement ps = connection.prepareStatement(sql)) {
        ps.setString(1, tableName);
        ps.setInt(2, score);
        ps.setInt(3, totalQuestionsAnswered);
        ps.setInt(4, durationSeconds);
        ps.executeUpdate();
        System.out.println("[QUIZ] attempt stored: " + tableName +
...
private void ensureSchema(Connection c) throws SQLException {
    ...
    try (Statement st = c.createStatement()) {
        st.executeUpdate(createAttempts);
    }

    // Helpful index for fast "best to beat" lookups
    // Not all MySQL versions support IF NOT EXISTS on CREATE INDEX; ignore if it already exists.
    try (Statement st = c.createStatement()) {
        st.executeUpdate(
            "CREATE INDEX idx_quiz_attempts_best " +
            "ON quiz_attempts (quiz_name, score DESC, duration_seconds ASC, finished_at ASC)"
        );
    } catch (SQLException ignore) {
        // index probably already exists; safe to ignore
    }
}
...
// small pause so the click is heard before right/wrong plays
private static final int SUBMIT_SFX_DELAY_MS = 180;

private void ensureSubmitIsWired() {
    ...
    submitButton.addActionListener(e -> {
        // 1) play the click
        try { 
            System.out.println("[QUIZ] submit click -> play " + SUBMIT_SFX);
            SoundUtil.playSound(SUBMIT_SFX); 
        } catch (Exception ex) {
            ex.printStackTrace();
        }

        // 2) delay the actual submission so click is audible
        new javax.swing.Timer(SUBMIT_SFX_DELAY_MS, t -> {
            ((javax.swing.Timer) t.getSource()).stop();
            handleSubmission();
        }).start();
    });

    // keep Enter key mapped to Submit
    SwingUtilities.invokeLater(() -> {
    ...
    private static void makeFlat(JRadioButton b) {
        b.setOpaque(false);
        b.setContentAreaFilled(false);
        b.setBorderPainted(false);
        b.setFocusPainted(false); // kill the dotted focus box
        b.setUI(new BasicRadioButtonUI()); // avoid LAFs that reintroduce outlines
    ...
    // Applies the white "submit" icons if available, harmless if not yet loaded
    ...
    /**
     * Tries several reasonable classpath paths (and last-resort file path) for an
     * image.
     */
    private ImageIcon findImageIconRobust(String rawPath) {
        ...
        while (p.startsWith("./"))
            p = p.substring(2);
        if (p.startsWith("/"))
            p = p.substring(1); // store without leading slash for joins
    ...
    /**
     * Apply the colored option button icons if we have them. Safe to call
     * repeatedly.
     */
    private void applyOptionIconsIfAvailable() {
        ...
        boolean haveAll = resizedImages != null && selectedResizedImages != null &&
                resizedImages.length >= 3 && selectedResizedImages.length >= 3 &&
                resizedImages[0] != null && resizedImages[1] != null && resizedImages[2] != null &&
                selectedResizedImages[0] != null && selectedResizedImages[1] != null
                && selectedResizedImages[2] != null;
        ...
        for (int i = 0; i < Math.min(3, options.length); i++) {
            ...
            // keep text (bird names) on the right; if you want icon-only, use
            // rb.setText("");
        }
    ...
    private BestEntry fetchBestFromDb() {
        ...
        final String sql = "SELECT score, total_questions, duration_seconds " +
                "FROM quiz_attempts " +
    ...
    /**
     * True if this run beats the previous best (higher score, or same score but
     * faster).
     */
    private boolean isNewRecord(int myScore, int myTotal, int mySeconds, BestEntry prev) {
        if (prev == null)
            return true;
        if (myScore > prev.score)
            return true;
        if (myScore < prev.score)
            return false;
    ...
    private void saveQuizResultToDatabase() {
        if (connection == null)
            return;
        final String sql = "INSERT INTO quiz_results (first_name, email, quiz_table, score, total_questions) " +
                "VALUES (?, ?, ?, ?, ?)";
    ...
    private void loadImageAndOptions(Question question) {
        ...
        if (optionsPanel != null && !optionsPanel.isVisible()) {
            optionsPanel.setVisible(true);
            optionsPanel.revalidate();
            optionsPanel.repaint();
        }

        // ... after setting image + options ...
        if (optionsPanel != null && !optionsPanel.isVisible()) {
            optionsPanel.setVisible(true);
            optionsPanel.revalidate();
            optionsPanel.repaint();
        ...
        // Now reveal Submit and wire Enter to it
        showSubmitUI(); // your helper: enables, shows, sets default button, revalidates
    ...
    private void finishQuiz() {
        ...
        if (countdownRing != null) {
            try {
                countdownRing.setSecondsRemaining(0);
            } catch (Exception ignore) {
            }
        }
        ...
        // persist + refresh
        saveAttemptToAttemptsTable(durationSeconds);
        refreshToBeatLabel();
        ...
        if (score == MAX_QUESTIONS) {
            ...
            if (finalMessageLabel != null) {
                finalMessageLabel.setText(
                        "Quiz completed! Your final score is " + score + "/" + totalQuestionsAnswered + suffix);
            }
        }
    ...
    private void restartQuiz() {
         hideSubmitUI(); // ensures the panel stays hidden until the first question binds
        ...
        try {
            ...
            if (countdownLabel != null) {
                countdownLabel
                        .setText("Time: " + String.format("%02d:%02d", remainingSeconds / 60, remainingSeconds % 60));
            }
        } catch (Exception ignore) {
        }
        ...
        if (finalMessageLabel != null) {
            finalMessageLabel.setText("");
            finalMessageLabel.setIcon(null);
        }
        if (scoreLabel != null)
        ...
        try {
            ...
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this, "Error loading questions: " + ex.getMessage(), "Error",
                    JOptionPane.ERROR_MESSAGE);
            if (submitButton != null)
                submitButton.setEnabled(false);
            if (submitPanel != null)
    ...
    private void handleSubmission() {
        ...
        if (quizFinished || questions == null || questions.isEmpty())
            return;
        ...
        if (isCorrect) {
            ...
            if (feedbackLabel != null)
                feedbackLabel.setText("Incorrect. The correct answer was: " + correct);
            try {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (src instanceof JRadioButton) {
            try {
                SoundUtil.playSound("radio_button_selected.wav");
            } catch (Exception ignored) {
            }
        }
    }
    ...
    private void processQuizSerialBuffer() {
        ...
        while ((nl = quizSerialBuf.indexOf("\n")) >= 0) {
            ...
            if (line.isEmpty())
                continue;
            if (line.startsWith("button pressed:") || line.startsWith("button released:"))
                continue;

            long now = System.currentTimeMillis();
            Long last = quizDebounceMap.get(line);
            if (last != null && (now - last) < QUIZ_DEBOUNCE_MS)
                continue;
    ...
    private void robustCloseQuizSerial() {
        try {
            if (quizPort == null)
                return;
            try {
                quizPort.removeDataListener();
            } catch (Throwable ignore) {
            }
            try {
                quizPort.setComPortTimeouts(SerialPort.TIMEOUT_NONBLOCKING, 0, 0);
            } catch (Throwable ignore) {
            }
            drainPort(quizPort);
            try {
    ...
    private void setSerialStatus(String text) {
        if (serialStatusLabel != null)
            serialStatusLabel.setText(text);
    }

    private void setResetEnabled(boolean enabled) {
        if (resetSerialButton != null)
            resetSerialButton.setEnabled(enabled);
    }

    private void reconnectSerial() {
        long now = System.currentTimeMillis();
        if (now - lastSerialAttemptMs < 1000)
            return;
    ...
    private static class CountdownRing extends JComponent {
        ...
        void setSecondsRemaining(int s) {
